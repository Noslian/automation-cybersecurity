        ...
        for client, keys in api_keys.items():
            ...
            if access_token:
                ...
                if response.status_code == 200:
                    ...
                    high_alert = [alert for alert in alerts if alert.get('Severity') == 'High' and alert.get('Name') not in ['GDPR data was shared publicly', 'GDPR data was shared with everyone in the organization']]
                    
                    if high_alert:
                        print(f"[HIGH] Existe {len(high_alert)} alertas nos últimos 20 minutos na console do Varonis do Cliente: {client}")
                    else:
